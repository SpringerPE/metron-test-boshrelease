...
var (
  ...
  origin = flag.String("origin", "fast", "Origin Envelope")
...
func main() {
    ...
    err := dropsonde.Initialize(*destination, *origin, "zz")
    if err != nil {
      fmt.Println("* Error initializing: " + err.Error())
      os.Exit(1)
    }
    fmt.Println("*** Init, sending to " + *destination + " with origin: " + *origin)
    pid := os.Getpid()
